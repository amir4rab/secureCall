...
const fileSlicer = async ( blob, chunksSize= 100000 ) => {
...
const generateFileMetaData = ( fileData, size, chunks, id ) => {
  return ({
    name: fileData.name,
...
export const fileReader = ( file, id ) => new Promise ( async (resolve, reject) => {
  const reader = new FileReader();
  reader.onload = async _ => {
    ...
    resolve({
      dataArr: slicedFile,
      metadata: generateFileMetaData( file, reader.result.byteLength, slicedFile.length, id )
    });
  };
